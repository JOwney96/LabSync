<x-guest-layout>
    <form method="POST" action="{{ route('register') }}" x-data="{ role: 'student' }">
        ...
        <div class="mt-4" x-show="role === 'faculty'" x-transition x-cloak>
            <x-input-label for="faculty_code" :value="__('Faculty Access Code')" />
            <x-text-input id="faculty_code" class="block mt-1 w-full" type="text" name="faculty_code" placeholder="Enter Faculty Code" />
            <x-input-error :messages="$errors->get('faculty_code')" class="mt-2" />
        </div>

        <div class="mt-4" x-show="role === 'admin'" x-transition x-cloak>
            <x-input-label for="admin_code" :value="__('Admin Registration Code')" />
            <x-text-input id="admin_code" class="block mt-1 w-full" type="text" name="admin_code" placeholder="Enter Admin Code" />
